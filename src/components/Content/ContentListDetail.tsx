...
const ContentListDetail = ({ onClose, contentId }: ContentDetailsProps) => {
  ...
  const [isUploadingVideo, setIsUploadingVideo] = React.useState(true);

  const validationSchema = yup.object().shape({
    name_id: yup.string().required().label('Name'),
    ...
    >
      {(formik) => (
        ...
        >
          ...
          <FormikInput name='name_id' label='Name' type='string' required />
          ...
          <div className='flex gap-5 mt-5 flex-row-reverse'>
            ...
            <Button
              type='submit'
              size='lg'
              disabled={!formik.dirty || !formik.isValid || isUploadingVideo}
            >
              Save
            </Button>
          </div>
