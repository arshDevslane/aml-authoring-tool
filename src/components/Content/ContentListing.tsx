...
const coloredDot = (
  info: CellContext<Content, unknown>,
  ...
  const columns: ColumnDef<Content>[] = useMemo(
    () => [
      ...
      },
      {
        ...
        cell: (info) =>
          toReadableFormat(
            (info.getValue() as Content['taxonomy'])?.board?.name?.en
          ),
        enableSorting: false,
      },
      {
      ...
      {
        accessorKey: 'l3_skills',
        header: 'L3 Skills',
        cell: ({ row }) =>
          row.original?.taxonomy?.l3_skill?.length
            ? row.original?.taxonomy?.l3_skill
                ?.map((l3) => l3.name.en)
                .join(', ')
            : '--',
        cellClassName: 'max-w-80 truncate',
